...
penalty = 10^3; %parameter to enforce boundary conditions for fluid, may have to tune for othe problems
...
%True solutions: v1 = exp(-t)v01, v2 = exp(-t)v02, wtilde = wt =
%exp(-t)v02(x,1), w = -exp(-t)v02(x,1), p = (1+exp(-t))(x-y)

%Fluid Lagrange degrees of freedom    
vn1 = 6*(y.^2 - y).*(x-1).^3.*x.^3; %Initial x component of velocity
vn2 = -3*(2*y.^3 - 3*y.^2).*x.^2.*(x-1).^2.*(2*x-1); %Initial y component of velocity
   
%Lagrange degrees of freedom   
wntilde(1:Ms) = 3*xs.^2.*(xs-1).^2.*(2*xs-1); %Initial plate velocity
wn(1:Ms) = -wntilde(1:Ms); %Initial plate displacement

%Hermite degrees of freedom
wntilde(Ms+1:dofs) = 6*xsherm.*(xsherm-1).*(5*xsherm.^2 - 5*xsherm + 1);
wn(Ms+1:dofs) = -1*wntilde(Ms+1:dofs);
...
clear x y xs ys xsherm i1 i2 dx r
...
for ell = 1:1:Lf
    ...
    %v2 Nonzero Dirichlet B.C.
     x = glbpts(:,1);
     y = glbpts(:,2);
     f1fvals = -6*(y.^2 - y).*(x-1).^3.*x.^3 + nu*(-12*(x - 1).^3.*x.^3 - 36*(y.^2 - y).*(x - 1).^3.*x - 108*(y.^2 - y).*(x - 1).^2.*x.^2 - 36*(y.^2 - y).*(x - 1).*x.^3);
     f2fvals = 3*(2*y.^3 - 3*y.^2).*x.^2.*(x-1).^2.*(2*x-1) + nu*(18*(x - 1).^3.*x.^2.*(2*y - 1) + 18*(x - 1).^2.*x.^3.*(2*y - 1) + 6*(2*y.^3 - 3*y.^2).*(x - 1).^3 + 54*(2*y.^3 - 3*y.^2).*(x - 1).^2.*x + 54*(2*y.^3 - 3*y.^2).*(x - 1).*x.^2 + 6*(2*y.^3 - 3*y.^2).*x.^3);
...
for ell = 1:1:Ls
      ...
      fsvals = -3*glbpts.^2.*(glbpts-1).^2.*(2*glbpts-1) - 720*glbpts + 360; 
...
%Handle the Hermite endpoints
     for r = 1:1:2
       Fs_tildetheta =  (0.5*h) * sum(PLATE_WTS.*fsvals.*theta(r,:));
       Fs_tildethetax =  (0.5*h)^2 * sum(PLATE_WTS.*fsvals.*theta(r+4,:));

       i = msh.LOCSTRUCT(ell,r);
       F0s(i) = F0s(i) + Fs_tildetheta;
       F0s(i+Ms) = F0s(i+Ms) + Fs_tildethetax;
       
       F0c(i) = F0c(i) + (0.5*h)*fcval * sum(PLATE_WTS.*theta(r,:));
       F0c(i + Ms) = F0c(i + Ms) + (0.5*h)^2 * fcval * sum(PLATE_WTS.*theta(r + 4,:));
     end
    %Handle the Lagrangian middle points
    i = msh.LOCSTRUCT(ell,3);
    Fs_tildetheta =  (0.5*h) * sum(PLATE_WTS.*fsvals.*theta(3,:));
    F0s(i) = F0s(i) + Fs_tildetheta;
    
    F0c(i) = F0c(i) + (0.5*h) * fcval * sum(PLATE_WTS.*theta(3,:));

    i = msh.LOCSTRUCT(ell,4);
    Fs_tildetheta =  (0.5*h) * sum(PLATE_WTS.*fsvals.*theta(4,:));
    F0s(i) = F0s(i) + Fs_tildetheta;
    
    F0c(i) = F0c(i) + (0.5*h) * fcval * sum(PLATE_WTS.*theta(4,:));
end
...
elseif strcmp(stepping, 'crank_nicolson')
    ...
    for tk = 2:1:fluid_time_steps
    RHSf(1:Mf) = 0.5*dtf*(exp(-tf)+exp(-(tf-dtf)))*F1f + 0.5*dtf*((1+exp(-tf))+(1+exp(-(tf-dtf))))*F1p + (Massf-0.5*nu*dtf*kf)*vn1;
    RHSf(Mf+1:2*Mf) = 0.5*dtf*(exp(-tf)+exp(-(tf-dtf)))*F2f + 0.5*dtf*((1+exp(-tf))+(1+exp(-(tf-dtf))))*F2p + (Massf-0.5*nu*dtf*kf)*vn2; 
...
plot_initdata = 0; %Plot the initial data
