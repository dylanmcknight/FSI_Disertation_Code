...
%% Define the portion of the boumdary with essential boundary conditions
% Omegaf = [];
% 
% for i = 1:1:M0f
%     if msh.POS(J0f(i),2) == 1
%         Omegaf = [Omegaf, J0f(i)];
%     end
% end
...
% MOmega = length(Omega);
% Omega_mid = setdiff(Omega,Omega_tilde);
% MOmega_mid = length(Omega_mid);
...
%This finds the associated heat triangular element for each linear
%structure element.
